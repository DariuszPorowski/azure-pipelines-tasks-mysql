...
Function Connect-MySqlServer {
    ...
    Begin {
        ...
        if ($Database) {
            $connectionString = "Server=$($Server);Port=$($Port);Uid=$($Username);Pwd=$($Password);database=$($Database);SslMode=Preferred;Connection Timeout=$($Timeout);";
        }
        else {
            $connectionString = "Server=$($Server);Port=$($Port);Uid=$($Username);Pwd=$($Password);SslMode=Preferred;Connection Timeout=$($Timeout);";
        }
    }
    Process {
        try {
